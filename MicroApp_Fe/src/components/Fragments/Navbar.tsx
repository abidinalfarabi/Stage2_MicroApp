        ...
        <div className="flex items-center gap-5">
          ...
          {userLogin === false ? (
            <div className="flex items-center">
              <button className="bg-white text-xl font-[700] px-7 py-1 rounded-md ml-5 hover:bg-black  hover:text-white border-2 border-white transition-all" onClick={() => setOpenLogin(true)}>
                LOGIN
              </button>
            </div>
          ) : (
            <div className="flex items-center">
              <button className="bg-white text-xl font-[700] h-11 w-11 flex justify-center items-center rounded-full ml-5">ABI</button>
            </div>
          )}

          <Modal open={openLogin} onClose={() => setOpenLogin(false)}>
            <Login onclick={loginUser} onClickRegist={handleRegister} getInputValue={getInputValue} />
          </Modal>
